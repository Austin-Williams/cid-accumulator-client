...
export function logConfig(contractAddress: string, config: AccumulatorClientConfig) {
	...
	console.log(`[Client] ⚙ Config: REMOTE_PIN_BASE_URL: ${config.REMOTE_PIN_BASE_URL ?? "undefined"}`)
	console.log(`[Client] ⚙ Config: REMOTE_PIN_HEADERS: ${config.REMOTE_PIN_HEADERS ? JSON.stringify(config.REMOTE_PIN_HEADERS) : "undefined"}`)
	console.log(`[Client] ⚙ Config: REMOTE_PIN_FAILURE_THRESHOLD: ${config.REMOTE_PIN_FAILURE_THRESHOLD ?? "undefined"}`)
